lapjv  <- function(costMat,resolution) {
     ...
     maxcost <- min(1e16,max(costMat))
     resolution <- 4*eps(maxcost)
     
     # Prepare working data
     costSize <- size(costMat)
     rdim <- costSize[1]
     cdim <- costSize[2]
     M <- min(min(costMat))
     ...
     if(cdim != rdim)
     {
          costMat <- rbind(costMat, (2*M+zeros(cdim-rdim,cdim)))
     }
     costMat[costMat != costMat] <- Inf # I think this is for sparse matrices, NaNs etc? Otherwise, seems odd.
     ...
     # column reduction
     for (j in rev(1:dim)) # reverse order gives better results
     {
          # find minimum cost over rows
          imin <- which.min(costMat[,j])
          v[j] <- costMat[,j][imin]
          if (!matches[imin])
          {
     ...
     u <- diag(costMat[,rowsol])-t(v[rowsol])
     u <- u[1:rdim]
     v <- v[1:cdim]
     cost <- sum(u)+sum(v[rowsol])
     costMat <- costMat[1:rdim,1:cdim]
     costMat <- costMat - matrix(rep(u,cdim),rdim,cdim) - matrix(rep(v,rdim),rdim,cdim, byrow=TRUE)
     if (swapf)
     {
          costMat <- t(costMat)
          t <- t(u)
     ...
     return(list(rowsol=rowsol,cost=cost,v=v,u=u,costMat=costMat))
}

print('Generating Costs')
duh <- magic(90)
print('Tracking')
# duh <- matrix(runif(25,0,1),5,5)
# duh <- matrix(c(6,5,1,0,7,5,8,1,9),3,3,byrow=FALSE)
px1 <- LinearAssignment(duh)
px2 <- lapjv(t(duh))$rowsol
px1
px2
px1 == px2

# Gets flipped somewhere. Figure it out.
duh[px1,] == duh[px2,]
